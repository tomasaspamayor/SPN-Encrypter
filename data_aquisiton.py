...
def generate_test_data(rows=100):
    """
    Generates 176-byte HEX strings and saves them to .txt files.

    - `keys.txt`: Contains random 176-byte HEX strings.
    - `tester.txt`: Contains a mix of fixed patterns and incremental HEX strings.
    - `tester_out.txt`: Contains random 176-byte HEX strings for output testing.
    """
...
def to_bin(hex_str):
    """
    Converts a hex string to a 1408-bit binary string.
    """
    try:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            dist = calculate_hamming_distance(
                ciphertexts_list[i], ciphertexts_list[j])
...
def calculate_shannon_entropy(list_of_keys):
    """
    Calculates entropy based on hex character frequency from a list of keys.
    """
...
def calculate_shannon_entropy_from_file(file_path):
    """
    Calculates entropy based on hex character frequency from a newline-delimited file.
    """
    keys_series = pd.read_csv(file_path, header=None, dtype=str)[0].dropna()
    return calculate_shannon_entropy(keys_series.tolist())
...
def plot_sac_values(sac_values, bit_length=1408):
    """
    Plots the Hamming distances calculated for the SAC analysis,
    with a reference line for the ideal SAC value.
    """
    plt.figure(figsize=(12, 6))
    plt.scatter(range(len(sac_values)), sac_values,
                color='teal', alpha=0.4, s=10)
    plt.axhline(y=bit_length/2, color='r', linestyle='--',
                label=f'Ideal SAC ({bit_length/2} bits)')
...
def plot_bigo_complexity(input_sizes, times_list):
    ...
    plt.scatter(input_sizes, times_list, color='blue',
                label='Actual Timing Data')
...
if __name__ == "__main__":
    ...
    print(
        f"Calculated Shannon Entropy: {calculate_shannon_entropy_from_file('shannon_entropy.txt'):.4f}"
    )
    ...
    byte_sizes = [count * 176 for count in block_counts]

    simulated_times = [0.0001 * n +
                       np.random.normal(0, 0.001 * n) for n in block_counts]
