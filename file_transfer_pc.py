...
class FileTransfer():
    ...
    def _check_bmp_compression(self, path):
        with open(path, 'rb') as f:
            f.seek(30) # Jump straight to the compression field
    ...
    def _convert_to_uncompressed_bmp(self, input_path, output_path):
        # Open the image (works for JPEG, PNG, or compressed BMP)
        img = Image.open(input_path)
        
        # Convert to RGB (removes alpha channels/transparency which can confuse PICs)
        img = img.convert("RGB")
        
        # Save as BMP. Pillow defaults to uncompressed BI_RGB.
        img.save(output_path, "BMP")
        print(f"Success! {output_path} is now a raw 24-bit uncompressed BMP.")
    ...
    def file_exchange(self):
        """
        Exchange files with a PIC18. Processes BMPs to be uncompressed,
        extracts pixel data, and handles the serial handshake.
        """
        if not os.path.exists(self.send_path):
            print(f"Error: File '{self.send_path}' not found.")
            return

        # 1. PRE-PROCESS: Convert BMP to uncompressed on disk BEFORE reading
        is_bmp = False
        ...
        # 3. PARSE BMP: Separate header from pixels for selective encryption
        if is_bmp:
            try:
        ...
        ser = None
        try:
            ser = serial.Serial(self.serial_port, self.baud_rate, timeout=20) # 20s timeout is safer
            ...
            with open(self.receive_path, output_mode, **output_kwargs) as f_recv:
                ...
                # 4. HANDSHAKE: SOT -> Mode -> Packet Count -> Ready
                if self.use_framing:
                ...
                for i in range(0, len(payload), self.packet_size):
                    ...
                    # Expect: 16 (data) + 4 (timers) + 176 (round keys)
                    ...
                    # PROGRESS TRACKER (Updates every 10 packets)
                ...
                # 6. CLEANUP & BMP RECONSTRUCTION
                print("\nTransfer complete. Cleaning up...")
                if timing_file: timing_file.close()
                if round_keys_file: round_keys_file.close()

                if is_bmp and bmp_recv_pixels:
                    with open(self.receive_path, 'wb') as out_bmp:
                        out_bmp.write(bmp_header)
                        # Trim padding to match original pixel count
